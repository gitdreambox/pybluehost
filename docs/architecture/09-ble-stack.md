...
class GATTServer:
    ...
    # ── 回调注册（供 Profile 层绑定 Characteristic 行为）──
    def on_read(self, uuid: UUID, handler: Callable[[], Awaitable[bytes]]) -> None: ...
    def on_write(self, uuid: UUID, handler: Callable[[bytes], Awaitable[None]]) -> None: ...
    def on_notify(self, uuid: UUID, handler: Callable[[], Awaitable[bytes]]) -> None: ...
    def on_indicate(self, uuid: UUID, handler: Callable[[], Awaitable[bytes]]) -> None: ...
```
