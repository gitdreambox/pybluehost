...
## 9. 子项目 B：PTS IUT（待单独 brainstorm）

简述（详细 spec 后续单独写）：

- PyBlueHost 当 **Implementation Under Test**，跑 Bluetooth SIG 官方 PTS（Profile Tuning Suite）一致性测试
- 目标：HCI / L2CAP / GAP / ATT / GATT / SMP test groups ≥ 90% 通过
- 需要：PTS dongle + SIG license（用户已有）
- 关键工作：
  - PTS Automation API（`ETSManager.dll` / PTSControl COM）控制
  - IUT control interface——PyBlueHost 接收 PTS 测试命令、驱动协议栈
  - MMI（Man-Machine Interface）自动应答——PTS 弹的操作提示自动响应
- 与子项目 A 无依赖；但 A 做完后 PTS 测试失败可结合 virtual sniffer 抓包 debug
...
- **v1.0**：已完成（31 Plans）
- **v1.1 子项目 A**：virtual sniffer（本 PRD，~5 周）
- **v1.1 子项目 B**：PTS IUT（单独 spec）
- **v2.0**：Classic Audio（已 brainstorm，[PRD-v2.0](PRD-v2.0.md)）
- v1.1 与 v2.0 无强依赖，可并行/穿插推进
