...
**当前进行中**：Plan 3a — USB Transport 核心  
...
- 完成时间：2026-04-15
- 提交范围：`pybluehost/transport/` + `tests/unit/transport/`（commit `c8a6353`→`44c3fda`）
- 测试：177 tests passed，transport 覆盖率 97%
- **补充遗漏项（待后续 Plan 补入）**：
  - `ReconnectConfig` dataclass（含 max_attempts、base_delay、max_delay）→ 并入 Plan 3a
  - `TransportSink.on_transport_error()` 回调方法 → 并入 Plan 3a（修改 base.py）
...
- 同时补充：`transport/base.py` 追加 `ReconnectConfig` 和 `on_transport_error`
