...
- **认领人**: Codex session
- **当前进度**: Task 1/2/3 完成；Task 4 已完成 Classic ACL waiter、L2CAP dynamic PSM connect/listen、SDPClient、RFCOMM outgoing/incoming SABM/UA、UIH dispatch、SPPClient connect 与 spp_echo listen 注册路径；CSR 硬件验收待执行
- **最后更新**: 2026-04-28
- **硬件验收记录**: `sdp-browser -t usb:vendor=csr -a A0:90:B5:10:40:82` 已走到 Classic ACL Create Connection，返回 `PAGE_TIMEOUT (0x04)`；`spp-echo -t usb:vendor=csr` 首次受控启动未立即崩溃，随后 CSR8510 出现 `Access denied` / `DRIVER_CONFLICT`，需要恢复设备访问后继续验收。
- **关键结论**:
