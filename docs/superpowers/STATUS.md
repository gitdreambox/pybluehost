...
- **认领人**：Codex session
- **开始时间**：2026-04-27
- **当前进度**：Task 2 完成 / Task 3 待执行（共 23 Task；Task 2 full-suite verification deferred by pre-existing hardware tests）
- **最后更新**：2026-04-27 22:01
- 已完成 Task：Task 1（loopback → virtual 公共 API 改名）、Task 2（LoopbackTransport 内化为 VirtualController._HCIPipe）
- 进行中 Task：无（下一步 Task 3：USBTransport.list_devices 与 bus/address 过滤）
- 验证记录：Task 2 targeted tests PASS；`uv run pytest tests/ -q --ignore=tests/hardware` PASS；`uv run pytest tests/ -q -m "not hardware"` blocked by existing Intel hardware timeout until marker tasks land.
