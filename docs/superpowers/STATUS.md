...
**当前进行中**：Pytest Transport Selection — Codex session，Task 23 部分完成，默认模式 full-suite 被本机不可用 USB 硬件阻塞
**下一步**：修复/禁用本机 Intel BE200 USB 硬件状态，或增强 autodetect 可用性探测后，重新执行 Task 23 Step 23.1
...
| Pytest Transport Selection | pytest transport 选择机制 | 🔄 进行中 | [pytest-transport-selection](plans/pytest-transport-selection.md) | `tests/conftest.py`, `tests/_transport_select.py`, `pybluehost/stack.py` |
...
|------|------|----------|----------|------|
| 2026-04-28 | Pytest Transport Selection Task 23 | 默认模式 full-suite 在本机未回落 virtual，因为 autodetect 找到 Intel BE200 + CSR8510；Intel BE200 当前 HCI 事件超时 / Access denied | 显式 virtual CI 路径已通过；默认模式 completion 阻塞，需修复/禁用硬件或增强 autodetect 可用性探测后重跑 Step 23.1 | ⚠️ 待确认 |
...
| 2026-04-27 | Pytest Transport Selection Task 2 | full-suite `uv run pytest tests/ -q` / `-m "not hardware"` verification blocked because current hardware tests still run Intel BE200 USB timeout paths | Task 2 verified with targeted tests and `uv run pytest tests/ -q --ignore=tests/hardware`; full-suite deferred until later transport marker tasks isolate hardware-only tests | ⚠️ 待后续 Task 处理 |
