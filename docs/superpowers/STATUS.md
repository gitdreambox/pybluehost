...
**当前状态**：v1.0 完结（31 Plans，Hardware E2E Readiness ✅）；**v2.0 Classic Audio ✅ 全部交付 + v2.1 Plan B.1（USB SCO quirk 适配）✅ + v2.1 Plan B.2（实时 PCM↔OS 音频）✅ + v1.1 Virtual Sniffer ✅（含真机实测）+ v1.2 Phase 1 ✅ + v1.2 Phase 2（auto-pts BTP）✅ 框架全交付（P.5-P.10，真机 pass-rate 留 operator）**
**下一步候选**：
1. **v1.2 PTS IUT Phase 2（auto-pts BTP 自动化）**——design spec（[`specs/2026-05-29-prd-v1.2-pts-iut-design.md`](specs/2026-05-29-prd-v1.2-pts-iut-design.md)）+ 6 个 Plan（P.5 BTP foundation / P.6 GAP / P.7 GATT / P.8 L2CAP+SMP / P.9 autopts project / P.10 PTS runs+CI）**均已就绪，待动工**。让 auto-pts 程序化驱动 IUT 跑完整 LE test case 集（HCI/L2CAP/GAP/GATT/SMP；Classic SDP/RFCOMM 永久留 Phase 1 REPL 手动）。Phase 1（PTS mode flags + IutActions + REPL + PICS 生成器 + runbook，15 Task）✅ 已实现并合并 master——见 [phase1 plan](plans/2026-05-31-v1.2-pts-iut-phase1.md)。**测试偏差**：action-layer（Task 6-10）与 CLI（12/14）的 per-task 单测文件未单独创建，改由 `tests/e2e/test_cli_app_lifecycle.py::test_pts_iut_cli_app_action_layer_drives_gap` 端到端覆盖。
...
| v1.2 PTS IUT Phase 1 | PTS mode flags（5 个）+ IutActions 动作层 + `app pts-iut` REPL + PICS 半自动生成器 + 操作员 runbook（15 Task） | ✅ 已合并 master（2026-06-22 cherry-pick；32 单测 + 1 e2e；测试覆盖偏差见 plan 顶部横幅） | [phase1](plans/2026-05-31-v1.2-pts-iut-phase1.md) | `pybluehost/pts/{config,actions,repl,pics_gen}.py`, `cli/app/pts_iut.py`, `cli/tools/pics_gen.py`, `ble/security.py`, `ble/smp.py`, `stack.py`, `docs/pts/`, `docs/PTS_RUNBOOK.md` |
| v1.2 Phase 2 P.5 | BTP foundation — frame codec + service ABC + Core service (upstream-aligned: LOG_MESSAGE + READ_BTP_MTU, no RESET_BOARD) + asyncio TCP tester + `app pts-tester` CLI | ✅ 完成（37 单测 + 2 e2e；upstream auto-pts master 2026-06-22 校准） | [p.5](plans/2026-06-01-v1.2-phase2-p5-btp-foundation.md) | `pybluehost/pts/btp/{__init__,opcodes,protocol,tester}.py`, `pybluehost/pts/btp/services/{base,core}.py`, `pybluehost/cli/app/pts_tester.py` |
| v1.2 Phase 2 P.6 | GAP BTP service — 20 opcodes（0x01-0x14：Read Supported/Controller Index/Info/Reset、Set Powered/Connectable/FastConn/Discoverable/Bondable、Start/Stop Advertising、Start/Stop Discovery、Connect/Disconnect、Set IO Cap、Pair/Unpair、Passkey Entry/Confirm Reply）+ 5 个 events（0x81-0x8B：DEVICE_FOUND/CONNECTED/DISCONNECTED/PAIRING_FAILED/SEC_LEVEL_CHANGED/PASSKEY_DISPLAY）；BTP→IutActions 翻译层；pts-tester CLI 注册 GapService + 钩入 stack.on_connection_event；2 e2e 真栈集成测试 | ✅ 完成（38 单测 + 2 e2e；upstream auto-pts master 2026-06-22 校准；漂移项 0x15-0x1A Privacy/OOB/MITM 显式不实现） | [p.6](plans/2026-06-01-v1.2-phase2-p6-gap-btp.md) | `pybluehost/pts/btp/services/gap.py`, `pybluehost/pts/btp/opcodes.py`, `pybluehost/cli/app/pts_tester.py` |
| v1.2 Phase 2 P.7 | GATT BTP service — server-build（0x02-0x08 Add Service/Char/Desc/Included/SetValue/StartServer/ResetServer）+ client（0x0a-0x1b：MTU、Discover All/By UUID、Read/Read Long/Read Multi、Write/Without Rsp/Long/Reliable、Cfg Notify/Indicate）+ events（0x80 Notification/Indication Received、0x81 Attribute Value Changed）；pending GATT db→stack ServiceDefinition 翻译；CCCD subscribe 写入；ATT notification 回调 → BTP 事件流；2 e2e 真栈集成 | ✅ 完成（37 单测 + 2 e2e；upstream auto-pts master 2026-06-22 校准；漂移项：plan 用 0x18/0x19 → 实际 0x1A/0x1B；plan 把 NOTIFY/INDICATE 当 server-push command → 改为 0x81 event；plan 漏 0x12 READ_UUID / 0x16 SIGNED_WRITE / 0x1C-0x1D GET_ATTRS 在 P.7 v1 显式不实现） | [p.7](plans/2026-06-07-v1.2-phase2-p7-gatt-btp.md) | `pybluehost/pts/btp/services/gatt.py`, `pybluehost/pts/btp/opcodes.py`, `pybluehost/ble/gatt.py`（添加 GATTClient.exchange_mtu + on_notification 公开包装）, `pybluehost/cli/app/pts_tester.py` |
| LE CoC manager 扩展 | v1.0 L2CAP 加 LE Credit-Based Channel signaling + manager API（前置 P.8 T2-T5）。6 Task：LE signaling PDU codec（0x06/0x07/0x14/0x15/0x16）+ dispatcher（_on_le_signaling 钩入 CID_LE_SIGNALING 固定通道）+ `connect_le_coc_channel` 出向 + `listen_le_coc_channel` 入向 + 信用 + 断开 + VirtualLELink e2e | ✅ 完成（33 单测 + 1 e2e；upstream-aligned；T5 顺便修了 T2 的潜在 `.peer_cid` vs `._peer_cid` bug） | [le-coc](plans/2026-06-23-le-coc-manager.md) | `pybluehost/l2cap/{le_signaling,manager}.py` |
| v1.2 Phase 2 P.8 | L2CAP BTP service — opcodes 0x01-0x05（Read Supported / Connect / Disconnect / Send Data / Listen）+ events 0x81/0x82/0x83（Connected / Disconnected / Data Received）；BTP chan_id(u8) ↔ LECoCChannel 映射；pts-tester CLI 注册 + 2 e2e（含 VirtualLELink 真栈 connect 触发 BTP event）；SMP T5/T6 删除（upstream 已无 btp_sm.txt，pairing 已在 P.6 GAP 服务里） | ✅ 完成（24 单测 + 2 e2e；upstream auto-pts master 2026-06-23 校准；漂移项：plan 用 11-byte Connect body / 0x03/0x04 wire 略错 / 0x06-0x0d ECFC 等显式不实现 / SMP service 整段废弃） | [p.8](plans/2026-06-08-v1.2-phase2-p8-l2cap-smp-btp.md) | `pybluehost/pts/btp/services/l2cap.py`, `pybluehost/pts/btp/opcodes.py`, `pybluehost/cli/app/pts_tester.py` |
| v1.2 Phase 2 P.9 | auto_pts_project IUT 模块 — `iutctl.py`（spawn `pybluehost app pts-tester` 子进程 + 等 BTP READY event）+ `pics.py`（按组加载 docs/pts/pics/*.draft.yaml）+ `pics_gen.yaml_draft_to_autopts_dict` 转换 helper + 提交 intel-BE200 参考 PICS 7 份 + `ixit.py`（GAP/GATT/L2CAP/SMP IXIT_* dicts，operator 改 TSPX_bd_addr_iut）+ wid/{gap,gatt,l2cap,sm}.py（继承上游 dispatch + 空 PYBLUEHOST_OVERRIDES）+ 操作员 README（quickstart + 真机 runbook） | ✅ 完成（19 单测；上游 autoptsclient 未安装时 WID 模块 graceful fallback；T6 本地 sanity / T7 真机 / T8 bug iter 留 operator 完成——硬件门控） | [p.9](plans/2026-06-08-v1.2-phase2-p9-autopts-project.md) | `auto_pts_project/pybluehost/{__init__,iutctl,pics,ixit}.py`、`auto_pts_project/pybluehost/wid/{__init__,gap,gatt,l2cap,sm}.py`、`auto_pts_project/pybluehost/README.md`、`pybluehost/pts/pics_gen.py`（appended helper）、`docs/pts/pics/*.draft.yaml`（7 份 intel-BE200 参考） |
| v1.2 Phase 2 P.10 | PTS runs + CI — `scripts/pts_matrix.py`（per-group YAML 结果矩阵：load/save/update/summary/render-summary/init）+ 4 个矩阵模板（GAP/GATT/L2CAP/SMP）+ `scripts/ci_btp_smoke.py`（spawn pts-tester、走 Core READY / Read Supported Services / GAP Set Powered / GATT Add Service 的 BTP 自检）+ `.github/workflows/pts-virtual.yml`（每 push/PR 跑 smoke + PTS 单测）+ `docs/pts/results/SUMMARY.md`（render-summary 输出，全部 0% untested 待真机填） | ✅ 框架完成（8 单测；smoke 本地通过；CI 待 GitHub 拉起验证；T2-T5 真机跑 + T7 真实 pass-rate 留 operator——硬件门控） | [p.10](plans/2026-06-08-v1.2-phase2-p10-pts-runs-ci.md) | `scripts/{pts_matrix,ci_btp_smoke}.py`、`scripts/__init__.py`、`docs/pts/results/{matrix/*.yaml,SUMMARY.md}`、`.github/workflows/pts-virtual.yml` |

> **v1.2 Phase 2 ✅ 框架全部交付（2026-06-23）**：P.5 BTP 基础 + P.6 GAP + P.7 GATT + P.8 L2CAP（含 LE CoC manager 扩展前置）+ P.9 auto_pts_project + P.10 矩阵工具与 CI。autoptsclient 现可指向 `auto_pts_project/pybluehost/` 把 PyBlueHost 当 IUT 跑。**真机硬件验证**（PTS dongle + Windows + autoptsserver）+ **每组实际 pass-rate** 留 operator 在 v1.2 真机 milestone 完成；框架本身 2165+ 单测全绿。
...
> **v1.2 PTS IUT Phase 1 ✅ 已合并 master（2026-06-22）**：让 PyBlueHost 当 PTS IUT 可测 + 手动驱动。15 Task：(1) `PTSModeConfig` 5 个 opt-in flag（disable_conn_updates / secure_pair_only / disable_sdp_on_le_pair / smp_options / smp_failure_at）+ `StackConfig.pts`（默认 None，零影响）；(2-5) 各 flag 的栈行为 wire（SC-only / SMP 字节覆盖 / SMP 失败注入 / 抑制 LE-pair 后 auto-SDP）；(6-10) `IutActions` 动作层（advertise/scan/connect/disconnect/pair/encrypt/notify/indicate/read/write/sdp_browse/rfcomm_open/l2cap_connect/set_io_cap/status）—— Phase 1 REPL 与 Phase 2 BTP tester 共用同一层；(11-12) `parse_repl_command` + `run_repl` + `app pts-iut` CLI；(13-14) PICS 半自动生成器 + `tools pics-gen` CLI；(15) `docs/PTS_RUNBOOK.md` + `docs/pts/{pics,ixit,results}` 脚手架。来源分支 `claude/elastic-aryabhata-eace93`（v2.0 音频之前分叉）cherry-pick 为 11 commit（`ca1c39b`..`95744fe`）+ 集成 commit `329096f`。验收：`tests/unit/pts/` 32 passed；全套 1941 passed / 36 skipped（唯一失败 `test_peer_stack_fixture_skips_with_clear_transport_error` 为 master 既有 bug，与 PTS 无关）。**测试覆盖偏差**：action-layer（Task 6-10）与 CLI（12/14）的 per-task 单测文件未单独创建，改由 `tests/e2e/test_cli_app_lifecycle.py::test_pts_iut_cli_app_action_layer_drives_gap` 端到端覆盖（亦守卫 IutActions 不随 Stack/GAP API 漂移）。Phase 2（auto-pts BTP）design spec + 6 个 Plan 已就绪待动工。

**总计：45 个 Plan（原 44 个 + v1.2 Phase 2 P.10 PTS runs + CI）**
