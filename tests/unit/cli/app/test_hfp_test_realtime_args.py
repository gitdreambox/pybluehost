"""hfp-test CLI: --mic-device / --speaker-device, mutually exclusive with WAV args."""
import argparse
import pytest
...
def test_hfp_test_wav_and_mic_are_mutually_exclusive():
    p = _build()
...
def test_hfp_test_out_and_speaker_are_mutually_exclusive():
    p = _build()
    with pytest.raises(SystemExit):
        p.parse_args([
            "hfp-test", "--role=hf", "--target=AA:BB:CC:DD:EE:FF",
...
def test_hfp_test_defaults_keep_old_wav_path_working():
    p = _build()
    args = p.parse_args([
        "hfp-test", "--role=hf", "--target=AA:BB:CC:DD:EE:FF",
        "--wav=in.wav",
