...
@patch("pybluehost.cli.tools.usb._libusb_library_path", return_value="libusb-1.0.dll")
@patch("pybluehost.cli.tools.usb.usb")
def test_cmd_diagnose_treats_intel_reset_pipe_error_as_reenumeration(
    mock_usb, mock_libusb, mock_input, capsys
):
    dev = _mock_usb_device(0x8087, 0x0036)
    rebooted_dev = _mock_usb_device(0x8087, 0x0036, addr=2)
    config = MagicMock()
    ...
    dev.get_active_configuration.return_value = config
    rebooted_dev.get_active_configuration.return_value = config
    pipe_error = Exception("Pipe error")
    pipe_error.errno = 32
    endpoint = MagicMock()
    endpoint.read.side_effect = [
        Exception("flush done"),
    ...
    out = capsys.readouterr().out
    assert "device disconnected/re-enumerating" in out
    assert "Intel firmware load skipped" in out
...
def test_cmd_diagnose_warns_firmware_load_needed_for_bootloader(
    ...
    assert result == 0
    out = capsys.readouterr().out
    assert "\x1b[33m[WARN]\x1b[0m Intel firmware load required:" in out
...
@patch("pybluehost.cli.tools.usb._load_intel_firmware_after_confirmation", return_value=True)
@patch("builtins.input", return_value="y")
@patch("pybluehost.cli.tools.usb._libusb_library_path", return_value="libusb-1.0.dll")
@patch("pybluehost.cli.tools.usb.usb")
def test_cmd_diagnose_prompts_and_starts_firmware_load(
    mock_usb, mock_libusb, mock_input, mock_load_fw, capsys
):
    dev = _mock_usb_device(0x8087, 0x0036)
    rebooted_dev = _mock_usb_device(0x8087, 0x0036, addr=2)
    config = MagicMock()
    ...
    assert result == 0
    mock_load_fw.assert_called_once_with(rebooted_dev)
    out = capsys.readouterr().out
    assert "Load Intel firmware now? [y/N]" in out
